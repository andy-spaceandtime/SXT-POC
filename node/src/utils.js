...
const apiContractSON = JSON.parse(fs.readFileSync(path.join(__dirname, './contract-abi/Api.json')));
const apiContract = new web3.eth.Contract(apiContractSON, apiContractAddress, {from: serviceAccount.address, gasLimit: 3000000});
const apiContractWss = new web3wss.eth.Contract(apiContractSON, apiContractAddress, {from: serviceAccount.address, gasLimit: 3000000});


// listen to an event
apiContractWss.events.ExecuteApi({}) //ExecuteApi is the event name
    .on('data', async function(event){
        
        let stringObj= JSON.stringify(event.returnValues);
        let jsonObj = JSON.parse(stringObj);

        axios.get(jsonObj.url) // here url is the request from the user.
